 ...
 */
async function updateEmployeeData(employees) {
  ...
  for (const employee of employees) {
    try {
      ...
      const updateData = {
        ...
        birthday: convertBirthDate(employee.birthDate),
      ...
      const errorMsg = `Error updating ${employee.employeeId}: ${error.message}`;
...
/**
 * Main function to update all employee data
 */
async function main() {
  console.log('=== Employee Data Update Script ===\n');
  
  console.log('Parsing employee data...');
  const employees = parseEmployeeData(employeeDataString);
  console.log(`Found ${employees.length} employees to update\n`);

  console.log('Updating Firestore...\n');
  const results = await updateEmployeeData(employees);
